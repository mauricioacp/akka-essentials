...
object ThreadModelLimitations extends App {
  ...
  /*
    MR #1: OOP encapsulation is only valid in the SINGLE THREADED MODEL.
  */
  ...
  /*
  MR #2: delegating something to a thread is a PAIN
   */
  ...
  /*
  MR #3: tracing and dealing with errors in a multithreading env is a PITN.
   */
  ...
  val futures = (1 to 9)
    ...
    .map(range => Future {
      if (range.contains(543548)) throw new RuntimeException("invalid member")
      range.sum
    })
